...
mod tests {
    ...
    #[test]
    fn header_footer_includes_app_blur_header_title_dim_rule() {
        let sheet = StyleSheet::parse(header_footer::DEFAULT_CSS);
        let mut found = false;
        for rule in sheet.rules() {
            let parts = rule.selector_chain().parts();
            if parts.len() == 2
                ...
                && parts[1].type_name() == Some("HeaderTitle")
            {
        ...
        assert!(
            found,
            "expected App:blur HeaderTitle default dimming rule for canonical parity"
        );
    }
