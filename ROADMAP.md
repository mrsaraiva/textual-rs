...
| Partial | Message bus | `Message` / `MessageEvent` + runtime message queue + bubble delivery via `Widget::on_message`. `Input` / `Checkbox` emit messages; some widgets still use direct callbacks. |
