...
| Done | RA-4 Typed handles | Landed 2026-06-13 (`3c5c4e7`..`5480d14`): `Handle<W>` + `HandleSlot<W>` in `src/handle.rs`; `QueryError::{Unmounted,TypeMismatch}`; `WidgetTree::tree_id` (cross-tree safety); mount-pipeline sink wiring; `AppRoot::with_child_handle`; app-level `query_one_typed`/`typed_handle`/`mount_typed`. All 5 examples migrated (judgment rule applied — handles used where clearer than stringly selectors; init-phase timing respected). Gate evidence: full suite 1,831/0; pty 8/8 (5 Pass / 2 XFail unchanged). |
